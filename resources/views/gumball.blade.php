                                    ...
                                    <tbody>
                                        @forelse ($faction->gumballs()->orderBy('name')->get() as $gumball)
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($gumball->userGumball(Auth::user()->id)->count())
                                                        {{ $gumball->userGumball(Auth::user()->id)->first()->updated_at }}
                                                    @else
                                                        -
                                                    @endif
                                                </td>
                                                <td>
                                                    <input type="checkbox" name="gumballs[]" value="{{ $gumball->id }}" {{ $user->gumballs()->where('gumball_id', '=', $gumball->id)->count() || old('gumball[' . $gumball->id . ']') ? 'checked' : '' }} />
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
